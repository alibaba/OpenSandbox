...
class SandboxPool internal constructor(
     ...
     * 3. If no idle and [policy] is [AcquirePolicy.FAIL_FAST], throws [PoolEmptyException].
     ...
     * @throws PoolEmptyException when policy is FAIL_FAST and no idle is available.
    ...
    private fun stopReconcile() {
        ...
        try {
            warmupExecutor?.awaitTermination(5, TimeUnit.SECONDS)
        } catch (_: InterruptedException) {
            Thread.currentThread().interrupt()
        }
        warmupExecutor = null
