...
public class SandboxE2ETests : IAsyncLifetime
{
    private readonly E2ETestFixture _fixture;
    private Sandbox? _sandbox;

    public SandboxE2ETests(E2ETestFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    public async Task InitializeAsync()
    {
        _sandbox = await Sandbox.CreateAsync(new SandboxCreateOptions
        {
            ...
            Metadata = new Dictionary<string, string> { ["tag"] = "e2e-test" },
    ...
    {
        Assert.NotNull(_sandbox);
        Assert.False(string.IsNullOrWhiteSpace(_sandbox.Id));
        Assert.True(await _sandbox.IsHealthyAsync());

        var info = await _sandbox.GetInfoAsync();
        Assert.Equal(_sandbox.Id, info.Id);
        Assert.Equal(SandboxStates.Running, info.Status.State);
        Assert.Equal(Constants.DefaultEntrypoint, info.Entrypoint);
        Assert.NotNull(info.Metadata);
        Assert.Equal("e2e-test", info.Metadata!["tag"]);
        Assert.True(info.ExpiresAt > info.CreatedAt);

        var endpoint = await _sandbox.GetEndpointAsync(Constants.DefaultExecdPort);
        AssertEndpointHasPort(endpoint.EndpointAddress, Constants.DefaultExecdPort);

        var metrics = await _sandbox.GetMetricsAsync();
        ...
        var renewResponse = await _sandbox.RenewAsync(5 * 60);
        Assert.NotNull(renewResponse);
        Assert.NotNull(renewResponse.ExpiresAt);
        var renewedInfo = await _sandbox.GetInfoAsync();
        ...
        try
        {
            Assert.Equal(_sandbox.Id, sandbox2.Id);
    ...
    public async Task Command_Execution_Success_Cwd_Background_Failure()
    {
        ...
        var echoResult = await _sandbox.Commands.RunAsync("echo Hello OpenSandbox E2E", handlers: handlers);
        ...
        var pwdResult = await _sandbox.Commands.RunAsync(
            "pwd",
        ...
        var failResult = await _sandbox.Commands.RunAsync(
    ...
    public async Task Filesystem_Operations_CRUD_Replace_Move_Delete()
    {
        ...
        await _sandbox.Files.CreateDirectoriesAsync(new[]
        {
            new CreateDirectoryEntry { Path = testDir1, Mode = 493 },
            new CreateDirectoryEntry { Path = testDir2, Mode = 420 }
        });

        var dirInfo = await _sandbox.Files.GetFileInfoAsync(new[] { testDir1, testDir2 });
        Assert.Equal(testDir1, dirInfo[testDir1].Path);
        Assert.Equal(493, dirInfo[testDir1].Mode);
        ...
        {
            new WriteEntry { Path = testFile1, Data = testContent, Mode = 420 },
            new WriteEntry { Path = testFile2, Data = Encoding.UTF8.GetBytes(testContent), Mode = 493 },
            new WriteEntry { Path = testFile3, Data = new MemoryStream(Encoding.UTF8.GetBytes(testContent)), Mode = 493 }
        });

        var readContent1 = await _sandbox.Files.ReadFileAsync(
            testFile1,
            new ReadFileOptions { Encoding = "utf-8" });
        var readContent1Partial = await _sandbox.Files.ReadFileAsync(
            testFile1,
            new ReadFileOptions { Encoding = "utf-8", Range = "bytes=0-9" });
        var readBytes2 = await _sandbox.Files.ReadBytesAsync(testFile2);
        var readContent2 = Encoding.UTF8.GetString(readBytes2);

        var chunks = new List<byte>();
        await foreach (var chunk in _sandbox.Files.ReadBytesStreamAsync(testFile3))
        {
        ...
        var fileInfoMap = await _sandbox.Files.GetFileInfoAsync(new[] { testFile1, testFile2, testFile3 });
        ...
        var searchResults = await _sandbox.Files.SearchAsync(new SearchEntry { Path = testDir1, Pattern = "*" });
        ...
        await _sandbox.Files.SetPermissionsAsync(new[]
        {
            new SetPermissionEntry { Path = testFile1, Mode = 493 },
            new SetPermissionEntry { Path = testFile2, Mode = 384 }
        });

        var updatedInfo = await _sandbox.Files.GetFileInfoAsync(new[] { testFile1, testFile2 });
        Assert.Equal(493, updatedInfo[testFile1].Mode);
        Assert.Equal(384, updatedInfo[testFile2].Mode);

        var beforeUpdate = (await _sandbox.Files.GetFileInfoAsync(new[] { testFile1 }))[testFile1];
        var updatedContent1 = testContent + " Appended line.";
        await Task.Delay(50);
        await _sandbox.Files.WriteFilesAsync(new[]
        {
            new WriteEntry { Path = testFile1, Data = updatedContent1, Mode = 420 }
        });

        var newContent1 = await _sandbox.Files.ReadFileAsync(testFile1, new ReadFileOptions { Encoding = "utf-8" });
        Assert.Equal(updatedContent1, newContent1);
        var afterUpdate = (await _sandbox.Files.GetFileInfoAsync(new[] { testFile1 }))[testFile1];
        AssertModifiedUpdated(beforeUpdate.ModifiedAt, afterUpdate.ModifiedAt, 1, 1000);

        await Task.Delay(50);
        await _sandbox.Files.ReplaceContentsAsync(new[]
        {
        ...
        var replaced = await _sandbox.Files.ReadFileAsync(testFile1, new ReadFileOptions { Encoding = "utf-8" });
        ...
        await _sandbox.Files.MoveFilesAsync(new[] { new MoveEntry { Src = testFile3, Dest = movedPath } });
        var movedBytes = await _sandbox.Files.ReadBytesAsync(movedPath);
        Assert.Equal(testContent, Encoding.UTF8.GetString(movedBytes));
        await Assert.ThrowsAnyAsync<Exception>(() => _sandbox.Files.ReadBytesAsync(testFile3));

        await _sandbox.Files.DeleteFilesAsync(new[] { testFile2 });
        await Assert.ThrowsAnyAsync<Exception>(() => _sandbox.Files.ReadFileAsync(testFile2));

        await _sandbox.Files.DeleteDirectoriesAsync(new[] { testDir1, testDir2 });
        var verify = await _sandbox.Commands.RunAsync(
    ...
    public async Task Command_Interrupt()
    {
        ...
        var executionTask = _sandbox.Commands.RunAsync("sleep 30", handlers: handlers);
        var id = await initLatch.Task.WaitAsync(TimeSpan.FromSeconds(15));

        await Task.Delay(2000);
        await _sandbox.Commands.InterruptAsync(id);
    ...
    public async Task Sandbox_Pause_And_Resume()
    {
        ...
        await _sandbox.PauseAsync();

        var pausedInfo = await WaitForStateAsync(_sandbox, SandboxStates.Paused, TimeSpan.FromMinutes(5));
        Assert.Equal(SandboxStates.Paused, pausedInfo.Status.State);

        var healthy = true;
        for (var i = 0; i < 10; i++)
        {
            healthy = await _sandbox.IsHealthyAsync();
            if (!healthy)
            {
                break;
            }
        ...
        var resumed = await _sandbox.ResumeAsync(new SandboxResumeOptions
        {
