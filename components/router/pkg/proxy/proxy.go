...
func (p *Proxy) parseSandboxHost(s string) (sandboxHost, error) {
	...
	if len(ingressAndPort) != 2 {
		return sandboxHost{}, fmt.Errorf("invalid host: %s", s)
	}
	return sandboxHost{ingressAndPort[0], ingressAndPort[1]}, nil
}
...
func (p *Proxy) fetchRealHost(host sandboxHost) (string, error, int) {
	...
	default:
		return "", fmt.Errorf("no sandboxes found for host: %s", host.ingressKey), http.StatusOK
	}
}
