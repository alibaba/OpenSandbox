...
// Unit tests here focus on annotation parsing logic and provider behavior

func TestParseEndpointsAnnotation(t *testing.T) {
	tests := []struct {
		name          string
		annotation    string
		expectedIPs   []string
		...
		{
			name:          "success - single IP",
			annotation:    `["10.244.1.5"]`,
			expectedIPs:   []string{"10.244.1.5"},
			expectedError: "",
		},
		{
			name:          "success - multiple IPs",
			annotation:    `["10.244.1.5", "10.244.1.6", "10.244.1.7"]`,
			expectedIPs:   []string{"10.244.1.5", "10.244.1.6", "10.244.1.7"},
			expectedError: "",
		},
		{
			name:          "error - empty annotation",
			annotation:    "",
			expectedIPs:   nil,
			expectedError: "endpoints annotation is empty",
		},
		{
		...
		},
		{
			...
			annotation:    `[]`,
			expectedIPs:   nil,
			expectedError: "endpoints annotation contains no IPs",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ips, err := parseEndpointsAnnotation(tt.annotation)
	...
	readyBatchSandbox := &sandboxv1alpha1.BatchSandbox{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				annotationEndpoints: `["10.0.0.1", "10.0.0.2"]`,
			},
		},
...
func TestBatchSandboxProvider_InvalidAnnotation(t *testing.T) {
	...
	assert.Error(t, err)
	assert.True(t, errors.Is(err, ErrSandboxNotReady), "Should return ErrSandboxNotReady")
	assert.Contains(t, err.Error(), "invalid endpoints annotation")
}
...
func TestBatchSandboxProvider_DynamicUpdate(t *testing.T) {
	...
	newBatchSandbox := &sandboxv1alpha1.BatchSandbox{
		ObjectMeta: metav1.ObjectMeta{
			...
			Namespace: namespace,
			Annotations: map[string]string{
				annotationEndpoints: `["10.0.0.100"]`,
			},
		},
		Spec: sandboxv1alpha1.BatchSandboxSpec{
			Replicas: ptr(int32(1)),
