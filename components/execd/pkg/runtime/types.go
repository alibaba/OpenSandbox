...
type bashSession struct {
	...
	// currentProcessPid tracks the PID of a short-lived run() command (fire-and-forget execution).
	// Set after cmd.Start(), cleared when run() returns. Used by close() for cleanup.
	currentProcessPid int

	// wsPid tracks the PID of the long-lived interactive shell started by Start/StartPTY.
	// Kept separate so run() cannot clobber it, ensuring IsRunning/close/interrupt remain correct.
	wsPid int
