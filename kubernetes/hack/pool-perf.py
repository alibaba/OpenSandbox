...
class PoolPerformanceTester:
    def __init__(self, pool_name, replicas_per_bsb, total_bsb_count):
        try:
    ...
    async def setup_pool(self):
        ...
        print(f"🚀 Setting up Pool: {self.pool_name} with size {self.total_bsb_count * self.replicas_per_bsb}...")
        try:
        ...
        while True:
            try:
                ...
                if available >= self.total_bsb_count * self.replicas_per_bsb:
                    print(f"✅ Pool is Ready. Available: {available}")
                    break
                print(f"Waiting for Pool... Available: {available}")
