...
func resolveMaxUnavailable(pool *sandboxv1alpha1.Pool, desiredTotal int32) int32 {
...
func (s *recreateUpdateStrategy) Compute(ctx context.Context, updateRevision string, pods []*corev1.Pod, idlePods []string) *UpdateResult {
	...
	maxUnavailable := resolveMaxUnavailable(s.pool, int32(len(pods)))
	...
	unavailableBudget := maxUnavailable - curUnavailable
	...
	sort.Slice(idlePodList, func(i, j int) bool {
